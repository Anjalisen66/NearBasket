...
const MyOrders = () => {
    ...
    className={`relative bg-white text-gray-500/70 ${order.items.length !== index + 1 && "border-b"} border-gray-300 flex flex-col md:flex-row md:items-center justify-between p-4 py-5 md:gap-16 w-full max-w-4xl`}>
        ...
        <div className='flex items-center mb-4 md:mb-0'>
            <div className=' p-4 rounded-lg'>
                {item.product?.image?.[0] ? (
                    ...
                    <div className='w-16 h-16 flex items-center justify-center text-gray-400 text-sm'>
                        No Image
                    </div>
                )}
            </div>
        ...
        <div className='flex flex-col justify-center md:ml-8 mb-4 md:mb-0'>
            ...
            <p>Status: {order.statue}</p>
            <p>Date: {new Date(order.createdAt).toLocaleDateString()}</p>
        </div>

        <p className='text-primary text-lg font-medium'>
            Amount: {currency} { ((Number(item?.product?.offerPrice) || 0) * (Number(item?.quantity) || 1)).toFixed(2) }
        </p>
    </div>
))}
