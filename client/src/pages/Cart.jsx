...
const Cart = () => {
    ...
    const getCart = () => {
        return Object.keys(cartItems).map((itemId) => {
            ...
            return null; // or skip it altogether
    ...
    const placeOrder = async () => {
      try {
        ...
        console.log("getcartamt:",getCartAmount());
                ...
                {cartArray.map((product, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        ...
                        <button onClick={() => removeFromCart(product._id)} className="cursor-pointer mx-auto">
                            <RxCrossCircled className="inline-block w-6 h-6 text-red-400" />
                        </button>
                    </div>)
                )}

                <button onClick={() => { navigate(`/products`); scrollTo(0, 0) }} className="group cursor-pointer flex items-center mt-8 gap-2 text-primary font-medium">
                    <MdKeyboardArrowLeft className="group-hover:-translate-x-1 transition w-6 h-6" />
                    Continue Shopping
                </button>
